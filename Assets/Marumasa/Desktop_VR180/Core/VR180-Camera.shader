Shader "Marumasa/VR180-Camera"
{
	...
	SubShader
	{
		...
		void surf( Input i, inout SurfaceOutput o )
		{
			...
			// 右目テクスチャからサンプリング用UV計算（UP/DOWNはXY反転）
			...
			float2 uvAtlasRightUP  = float2( ( 1.0 - uvUp.x ) * 0.25 + atlasUP_offsetX, 1.0 - uvUp.y );
			float2 uvAtlasRightDOWN = float2( ( 1.0 - uvDown.x ) * 0.25 + atlasDOWN_offsetX, 1.0 - uvDown.y );
